...
/** add sw timer for charging*/
//cpu boost
#include <linux/kthread.h>
#include <linux/hrtimer.h>
#include <linux/sched.h>
static struct hrtimer term_charging_timer;
int term_charging_timer_active = 0;
static DECLARE_WAIT_QUEUE_HEAD(term_charging_timer_waiter);
static struct task_struct *term_charging_thread = NULL;
static bool term_charging_timer_flag = false;
...
//#define LYZ_DEFINE_LOG   //liyizeng add for debug 
#define LYZ_DEFINE_CHARGE_EDIT_SOC   //liyizeng add for charger edit new SOC for asus
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"soc_final=%d soc_ocv=%d soc_cutoff=%d ocv_uv=%u batt_temp=%d\n",
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug %u raw converted into %lld uv\n", reading, uv);
#endif
	pr_debug("%u raw converted into %lld uv\n", reading, uv);

	uv = adjust_vbatt_reading(chip, uv);
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug adjusted into %lld uv\n", uv);
#endif
...
{
	int rc;
#if 1//lyz for three time for debounce
	int last_ocv_uv_temp[5];
	int i,j,temp_uv,temp_last_ocv;
	for(i=0;i<5;i++)
	{
	#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug calib_vadc start \n");
	#endif
	rc = calib_vadc(chip);
	if (rc)
		...
		msleep(50);
	#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug convert_and_store_ocv 1111 last_ocv_uv_temp=%d i=%d",last_ocv_uv_temp[i],i);
	#endif
	}
	for(j=0;j<5;j++)
	{
	...
	for(i=0;i<5;i++)
	{
		printk(KERN_ERR"lyz debug convert_and_store_ocv 2222 last_ocv_uv_temp=%d i=%d\n",last_ocv_uv_temp[i],i);
	}
#endif
	temp_last_ocv = ( last_ocv_uv_temp[1]+last_ocv_uv_temp[2]+last_ocv_uv_temp[3])/3;
#ifdef LYZ_DEFINE_LOG
printk(KERN_ERR"lyz debug convert_and_store_ocv chip->last_ocv_uv=%d,temp_last_ocv=%d \n",chip->last_ocv_uv,temp_last_ocv);
#endif
//	chip->last_ocv_uv =( last_ocv_uv_temp[1]+last_ocv_uv_temp[2]+last_ocv_uv_temp[3])/3;
...
//	printk(KERN_ERR"lyz debug convert_and_store_ocv chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#else
	rc = calib_vadc(chip);
	if (rc)
		pr_err("Vadc reference voltage read failed, rc = %d\n", rc);
...
#endif
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug convert_and_store_ocv end chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#endif
}
...
#ifdef LYZ_DEFINE_LOG
 printk(KERN_ERR"lyz debug read_and_update_ocv chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#endif
	...
	if ((chip->last_ocv_raw == OCV_UNINITIALIZED) ||
			...
			{
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug ocv_raw=0x%x last_ocv_raw=0x%x last_ocv_uv=%d\n",
		ocv_data, chip->last_ocv_raw, chip->last_ocv_uv);
		printk(KERN_ERR"lyz debug new OCV!\n");	
#endif
		pr_debug("new OCV!\n");
		chip->last_ocv_raw = ocv_data;
...
/*add sw timer for charging*/
#define TERM_CHARGING_TIMER 5
#ifdef LYZ_DEFINE_CHARGE_EDIT_SOC
int time_charge_10=0;
int time_charge_0=0;
#endif
int term_charging_timer_sw_thread_handler(void *unused)
{
	int rc = -EINVAL;
	...
	time = TERM_CHARGING_TIMER*60;
	ktime = ktime_set(time, 0);
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"zmlin term_charging_timer_sw_thread_handler\n");
#endif
	printk(" term_charging_timer_sw_thread_handler\n");
	
	do {
		wait_event_interruptible(term_charging_timer_waiter,(term_charging_timer_flag == true));
		term_charging_timer_flag = false;
		
		batt_psy = power_supply_get_by_name("battery");
		bms_psy = power_supply_get_by_name("bms");
		chip = container_of(bms_psy,
					struct qpnp_bms_chip, bms_psy);
...
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug ocv:%d present:%d\n",chip->last_ocv_uv,present);
#endif
		printk("  ocv:%d present:%d\n",chip->last_ocv_uv,present);
		//edit for charger end
#ifdef LYZ_DEFINE_CHARGE_EDIT_SOC
		if((chip->last_ocv_uv >= 4350000)&&(batt_temp >= 100)){//stop
		time_charge_10++;
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz term_charging_timer_sw_thread_handler 1 time_charge_10:%d \n",time_charge_10);
#endif
		if(time_charge_10>3)
		{
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz term_charging_timer_sw_thread_handler 2 time_charge_10:%d \n",time_charge_10);
#endif
		time_charge_10 = 0;
		   if (batt_psy){
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug handler set ok\n");
#endif
		   	printk(KERN_ERR"handler set ok\n");
		...
		{
			if(present)
				hrtimer_start(&term_charging_timer, ktime, HRTIMER_MODE_REL);
			else
				hrtimer_cancel(&term_charging_timer);
		}
		   //term_charging_timer_active = 0;
		}
		//edit for charger end
		else if((chip->last_ocv_uv >= 4342000)&&(batt_temp < 100)){//stop
		time_charge_0++;
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz term_charging_timer_sw_thread_handler 1 time_charge_0:%d \n",time_charge_0);
#endif
		if(time_charge_0>3)
		{
#ifdef LYZ_DEFINE_LOG
...
#ifdef LYZ_DEFINE_LOG
				printk(KERN_ERR"lyz debug term_charging_timer_sw_thread_handler handler hrtimer_start\n");
#endif
		        printk(KERN_ERR"handler hrtimer_start\n");
			if(present)
				hrtimer_start(&term_charging_timer, ktime, HRTIMER_MODE_REL);
			else
				hrtimer_cancel(&term_charging_timer);
...
#else
		if((chip->last_ocv_uv >= 4346000)&&(batt_temp >= 100)){//stop
		   if (batt_psy){
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug handler set ok\n");
#endif
		   	printk("handler set ok\n");
			ret.intval = POWER_SUPPLY_STATUS_FULL;
			rc = batt_psy->set_property(batt_psy,
					POWER_SUPPLY_PROP_STATUS, &ret);
			if (rc) {
		...
		//edit for charger end
		else if((chip->last_ocv_uv >= 4338000)&&(batt_temp < 100)){//stop
		   if (batt_psy){
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug handler set ok\n");
#endif
...
enum hrtimer_restart term_charging_hrtimer_func(struct hrtimer *timer)
{
	
	
	printk("zmlin [term_charging_hrtimer_func]\n");

	term_charging_timer_flag = true;
	wake_up_interruptible(&term_charging_timer_waiter);

	return HRTIMER_NORESTART;
}
//10min timer
int term_charging_timer_start(struct qpnp_bms_chip *chip)
{
    unsigned long time = 0;
    int rc = -EINVAL;
    ktime_t ktime;
    ...
    time = TERM_CHARGING_TIMER*60;
    ktime = ktime_set(time, 0);
    rc = hrtimer_cancel(&term_charging_timer);
    rc = get_batt_therm(chip, &batt_temp);
    if (rc)
	batt_temp = BMS_DEFAULT_TEMP;
    printk("zmlin term_charging_timer:%d,temp:%d\n",rc,batt_temp);
	
    term_charging_timer_active = 1;
    printk("zmlin ocv:%d\n",chip->last_ocv_uv);
	//edit for charger end
#ifdef LYZ_DEFINE_CHARGE_EDIT_SOC
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR" lyz term_charging_timer_start hrtimer_start \n");
#endif
	hrtimer_start(&term_charging_timer, ktime, HRTIMER_MODE_REL);
#else
   	if((chip->last_ocv_uv >= 4346000)&&(batt_temp >= 100)){//stop
...
EXPORT_SYMBOL(term_charging_timer);
...
static int report_eoc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->batt_psy) {
		...
		if (rc) {
			...
			if(0 == term_charging_timer_active)
			term_charging_timer_start(chip);
			//ret.intval = POWER_SUPPLY_STATUS_FULL;
...
static void check_eoc_condition(struct qpnp_bms_chip *chip)
{
	...
	if ((chip->last_soc == 100) &&
			(status == POWER_SUPPLY_STATUS_CHARGING)&&(0 == term_charging_timer_active))
		chip->ocv_at_100 = -EINVAL;
...
//liyizeng edit start for bug 198346 20160815
/*
...
*/
//liyizeng edit end for bug 198346 20160815
...
static int warm_condition_flag = 0;
static int check_thermal_condition(struct qpnp_bms_chip *chip)
{
	int rc = -EINVAL;
	union power_supply_propval ret = {0,};
	...
	if (chip->batt_psy) {
		...
		if (rc) {
			pr_err("Unable to get battery 'HEALTH' rc=%d\n", rc);
		} else if (ret.intval == POWER_SUPPLY_HEALTH_WARM) {
			printk("Report EOC to charger for warm condition:%d\n",chip->last_ocv_uv);
			if((chip->last_ocv_uv >= 4092000)&&(0 == warm_condition_flag)){
			warm_condition_flag = 1;
			ret.intval = 0;
			rc = chip->batt_psy->set_property(chip->batt_psy,
					POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
			}
			else if((chip->last_ocv_uv <= 4000000)&&(1 == warm_condition_flag)){
			printk("Report EOC to recharger for warm condition:%d\n",chip->last_ocv_uv);
			warm_condition_flag = 0;
			ret.intval = 1;
			rc = chip->batt_psy->set_property(chip->batt_psy,
					POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
			}
		...
		else if((1 == warm_condition_flag)&&(ret.intval == POWER_SUPPLY_HEALTH_GOOD)){
			warm_condition_flag = 0;
			ret.intval = 1;
			rc = chip->batt_psy->set_property(chip->batt_psy,
					POWER_SUPPLY_PROP_CHARGING_ENABLED, &ret);
...
static int soc_shutdown_voltage_limit(struct qpnp_bms_chip *chip)
{
	int rc = -EINVAL;
	static int cnt0 = 0;
	static int flag = 0;
	...
	if (chip->batt_psy) {
		...
		printk(KERN_ERR"vbat:%d\n",ret.intval);
		if (rc) {
			pr_err("Unable to get battery 'STATUS' rc=%d\n", rc);
			//return rc;
		}else if(ret.intval > 3300000){//uv
			chip->last_soc = 1;//1% soc
		}else{
		    	cnt0++;
			if(0 == flag)
			chip->last_soc = 1;//1% soc
			...
			cnt0 = 0;
			flag = 1;
		    	chip->last_soc = 0;//0% soc
		    	printk(KERN_ERR"last_soc:%d****\n",chip->last_soc);
			}
		}
	}
...
//liyizeng add
#ifdef LYZ_DEFINE_CHARGE_EDIT_SOC 
	if((is_charger_present(chip))&&((soc>30)&&(soc<97)))
	{
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug start soc=%d \n",soc);
#endif
		switch(soc)
		{
			case 31:
...
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug end soc=%d\n ",soc);
#endif
//		return soc;
	}
#endif
...
#if 0//def LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug charging=%d last_soc=%d last_soc_unbound=%d\n",
...
#ifdef LYZ_DEFINE_CHARGE_EDIT_SOC
#ifdef LYZ_DEFINE_LOG
//	printk(KERN_ERR"lyz debug last_soc=%d charging=%d soc=%d soc_change=%d ,charger_present:%d \n",chip->last_soc, charging,soc, soc_change,is_charger_present(chip));		
#endif
...
#ifdef LYZ_DEFINE_LOG
//	printk(KERN_ERR"lyz debug last_soc=%d charging=%d soc=%d \n",chip->last_soc, charging,soc);	
#endif
#else
		if (soc < chip->last_soc && soc != 0)
			soc = chip->last_soc - soc_change;
	...
	printk(KERN_ERR"last_soc=%d calculated_soc=%d soc=%d time_since_last_change=%d\n",
	...
	//if((0 == chip->last_soc)&&(!is_charger_present(chip)))
	if(0 == chip->last_soc)	
		soc_shutdown_voltage_limit(chip);
	printk(KERN_ERR"Reported SOC=%d,charger_present=%d\n", chip->last_soc,is_charger_present(chip));
	//add for thermal monitor
	if(is_charger_present(chip))
		check_thermal_condition(chip);
...
//liyizeng edit end for bug 198346 20160815
#ifdef LYZ_DEFINE_LOG
//printk(KERN_ERR"lyz debug report_state_of_charge report_vm_bms_soc \n");
#endif
...
#ifdef LYZ_DEFINE_LOG
//			printk(KERN_ERR"lyz debug monitor_soc_work lookup_soc_ocv \n");
#endif
			...
			if (chip->calculated_soc != new_soc) {
				...
				chip->catch_up_time_sec = 0;
#ifdef LYZ_DEFINE_LOG
//				printk(KERN_ERR"lyz debug monitor_soc_work 1111 report_vm_bms_soc chip->calculated_soc:%d \n",chip->calculated_soc);
#endif
...
#ifdef LYZ_DEFINE_LOG
//				printk(KERN_ERR"lyz debug monitor_soc_work 2222 report_vm_bms_soc \n");
#endif
				report_vm_bms_soc(chip);
			}
		}
...
#ifdef LYZ_DEFINE_LOG
		 printk(KERN_ERR"lyz debug qpnp_vm_bms_power_set_property chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#endif
		pr_debug("OCV = %d\n", val->intval);
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug bms_new_battery_setup chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#endif
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug read_and_populate_fifo_data convert_vbatt_raw_to_uv \n ");
#endif
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug read_and_populate_acc_data convert_vbatt_raw_to_uv \n ");
#endif
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug bms_fifo_update_done_irq_handler read_and_populate_fifo_data \n ");
#endif
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug bms_fsm_state_change_irq_handler read_and_populate_fifo_data \n ");
#endif
	rc = read_and_populate_fifo_data(chip);
	if (rc) {
		pr_err("Unable to read FIFO data rc=%d\n", rc);
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug bms_fsm_state_change_irq_handler read_and_populate_acc_data \n ");
#endif
...
//liyizeng add for bug 192931 20160718	
//	{
//		pr_debug("shutdown OCV %d - invalid\n", stored_ocv);
//		chip->shutdown_ocv = OCV_INVALID;
//		chip->shutdown_soc = SOC_INVALID;
...
#ifdef LYZ_DEFINE_LOG
		printk(KERN_ERR"lyz debug adjust_pon_ocv chip->last_ocv_uv=%d ",chip->last_ocv_uv);
#endif
	}
}
...
static int calculate_initial_soc(struct qpnp_bms_chip *chip)
{
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug calculate_initial_soc start chip->last_ocv_uv=%d ",chip->last_ocv_uv);
#endif
	...
	if (chip->warm_reset) {
...
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug calculate_initial_soc 111 lookup_soc_ocv \n");
#endif
...
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug calculate_initial_soc 222 lookup_soc_ocv \n");
#endif
...
#ifdef LYZ_DEFINE_LOG
		 printk(KERN_ERR"lyz debug calculate_initial_soc 333 lookup_soc_ocv \n");
#endif
		chip->calculated_soc = lookup_soc_ocv(chip,
		...
		if (!chip->shutdown_soc_invalid &&
			...
			chip->last_soc = chip->shutdown_soc;
#ifdef LYZ_DEFINE_LOG
			printk(KERN_ERR"lyz debug calculate_initial_soc 444 lookup_soc_ocv \n");
#endif
			chip->calculated_soc = lookup_soc_ocv(chip,
						chip->shutdown_ocv, batt_temp);
			pr_debug("Using shutdown SOC\n");
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	schedule_delayed_work(&chip->monitor_soc_work, 0);
        ...
        /*add sw timer for charging*/
	hrtimer_init(&term_charging_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    	term_charging_timer.function =term_charging_hrtimer_func;
    	term_charging_thread = kthread_run(term_charging_timer_sw_thread_handler, NULL,
			"term_charging_timer");
	if (IS_ERR(term_charging_thread)) {
		printk( "[%s]: failed to term_charging_thread thread\n",
				    __func__);
	}
	/*
...
static void process_suspend_data(struct qpnp_bms_chip *chip)
{
	int rc;
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug process_suspend_data chip->last_ocv_uv=%d ",chip->last_ocv_uv);
#endif
	mutex_lock(&chip->bms_data_mutex);
	...
	memset(&chip->bms_data, 0, sizeof(chip->bms_data));
#ifdef LYZ_DEFINE_LOG
//	printk(KERN_ERR"lyz debug process_suspend_data read_and_populate_fifo_data \n ");
#endif
	rc = read_and_populate_fifo_data(chip);
	if (rc)
		pr_err("Unable to read FIFO data rc=%d\n", rc);
#ifdef LYZ_DEFINE_LOG
//	printk(KERN_ERR"lyz debug process_suspend_data read_and_populate_acc_data \n ");
#endif
	rc = read_and_populate_acc_data(chip);
	if (rc)
		pr_err("Unable to read ACC_SD data rc=%d\n", rc);
...
#ifdef LYZ_DEFINE_LOG
	printk(KERN_ERR"lyz debug process_resume_data chip->last_ocv_uv=%d ",chip->last_ocv_uv);
#endif
	rc = get_batt_therm(chip, &batt_temp);
	if (rc < 0) {
		pr_err("Unable to read batt temp, using default=%d\n",
...
#ifdef LYZ_DEFINE_LOG
//       printk(KERN_ERR"lyz debug process_resume_data 1111 chip->last_ocv_uv=%d \n",chip->last_ocv_uv);
#endif
	if (old_ocv != chip->last_ocv_uv) {
		...
		memset(&chip->bms_data, 0, sizeof(chip->bms_data));
#ifdef LYZ_DEFINE_LOG
//		printk(KERN_ERR"lyz debug process_resume_data lookup_soc_ocv \n");
#endif
		chip->calculated_soc = lookup_soc_ocv(chip,
				chip->last_ocv_uv, batt_temp);
