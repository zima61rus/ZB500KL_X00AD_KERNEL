...
/*charging flag*/
extern int term_charging_timer_active;
/* usb_interrupts */
...
#if 0//liyizeng add for bug 223254 start
	if (!chip->cfg_btc_disabled)
#endif//liyizeng add for bug 223254 end
...
{
//add for bug180413 by liyizeng start 20160829
#if 1//
	union power_supply_propval ret = {0,};
	int last_ocv_uv;
//	printk(KERN_ERR"lyz debug get_prop_battery_voltage_now start  \n");

	if (chip->bms_psy) {
		chip->bms_psy->get_property(chip->bms_psy,
				POWER_SUPPLY_PROP_VOLTAGE_OCV, &ret);
		last_ocv_uv = ret.intval;
//		printk(KERN_ERR"lyz debug get_prop_capacity last_ocv_uv:%d  \n",last_ocv_uv);
		return last_ocv_uv;
	} else {
//		printk(KERN_ERR"lyz debug No BMS supply registered return \n");
	}
      return 0;
//add for bug180413 by liyizeng end 20160829
#else
	int rc = 0;
...
/*
static int get_prop_current_level_now(struct qpnp_lbc_chip *chip)
{
	union power_supply_propval ret = {0,};
	int rc,type = 0;
	...
	if (rc)
        	dev_err(chip->dev,
            		"Couldn't read get_prop_current_level_now, rc=%d\n", rc);	
         else{
		type = ret.intval;
		printk("zmlin get_prop_current_level_now type:%d\n",type);
...
static int get_prop_batt_temp(struct qpnp_lbc_chip *chip)
{
	...
	if(chip->usb_psy){
		...
		if (rc){
        	dev_err(chip->dev,
            		"Couldn't read get_prop_batt_temp, rc=%d\n", rc);	
        }else{
			printk("get_prop_batt_temp type:%d\n",ret.intval);
			if((ret.intval == POWER_SUPPLY_TYPE_USB_DCP)&&(chip->bat_is_hot != 1))
				results.physical += 15;	
			else 
				if(1 == chip->bat_is_hot) results.physical -= 10;
			if((ret.intval == POWER_SUPPLY_TYPE_USB)&&(chip->bat_is_hot != 1))
				results.physical += 5;	
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		//val->intval = get_prop_current_level_now(chip);
		printk("zmlin current:%d\n",val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	bool bat_hot = 0,bat_warm = 0, bat_cool = 0,bat_cold = 0;
	int temp;
	unsigned long flags;
	...
	temp = get_prop_batt_temp(chip);
        //chip->cfg_bat_present_decidegc = -300;
	printk("temp = %d state = %s\n", temp,
			state == ADC_TM_WARM_STATE ? "hot" : "cold");

        printk("%s : chip->hot_bat_decidegc = %d, chip->warm_bat_decidegc = %d, chip->cool_bat_decidegc = %d, chip->cold_bat_decidegc = %d,chip->cfg_bat_present_decidegc = %d\n", 
            __func__, chip->cfg_hot_bat_decidegc, chip->cfg_warm_bat_decidegc, chip->cfg_cool_bat_decidegc, chip->cfg_cold_bat_decidegc, chip->cfg_bat_present_decidegc);
#if 0
 ...
 if (state == ADC_TM_WARM_STATE) {
	    ...
	    printk("zmlin warm hot\n");
            ...
            chip->adc_param.low_temp =
                chip->cfg_cool_bat_decidegc;//-50
            chip->adc_param.high_temp =
                chip->cfg_warm_bat_decidegc;//550
			printk("zmlin warm cool\n");
        } else if (temp >=
            ...
            chip->adc_param.state_request =
                    ADC_TM_HIGH_LOW_THR_ENABLE;
			printk("zmlin warm cold\n");
        } else if (temp >= chip->cfg_bat_present_decidegc) {
            bat_hot = false;
            bat_warm = false;
            bat_cold = true;
            bat_cool = false;

            chip->adc_param.high_temp = chip->cfg_cold_bat_decidegc;
            chip->adc_param.low_temp = chip->cfg_bat_present_decidegc;
            chip->adc_param.state_request =
                    ADC_TM_HIGH_LOW_THR_ENABLE;
			printk("zmlin warm present\n");
        }
    } 
	else {
        if (temp <= chip->cfg_bat_present_decidegc) {
            bat_cold = true;
            bat_cool = false;
            bat_hot = false;
            bat_warm = false;
            ...
            bat_cool = false;
            chip->adc_param.high_temp =
                chip->cfg_cold_bat_decidegc;//-50
            /* add low_temp to enable batt present check */
            chip->adc_param.low_temp =
                chip->cfg_bat_present_decidegc;//-300
            chip->adc_param.state_request =
                ADC_TM_HIGH_LOW_THR_ENABLE;
			printk("zmlin else cold\n");
            ...
            chip->adc_param.state_request =
                    ADC_TM_HIGH_LOW_THR_ENABLE;
			printk("zmlin else hot\n");
        }
    }
        if (bat_hot ^ chip->bat_is_hot || bat_cold ^ chip->bat_is_cold) {
	...
	printk("hot %d ,warm %d, cool %d, cold %d ,low = %d deciDegC, high = %d deciDegC\n",
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
	...
	OF_PROP_READ(chip, cfg_bat_present_decidegc, "bat-present-decidegc", rc, 1);
	...
	printk("vddmax-mv=%d, vddsafe-mv=%d, vinmin-mv=%d, ibatsafe-ma=$=%d\n",
			chip->cfg_max_voltage_mv,
			chip->cfg_safe_voltage_mv,
			chip->cfg_min_voltage_mv,
			chip->cfg_safe_current);
	printk("bat-present-decidegc=%d,hot-bat-decidegc=%d,warm-bat-decidegc=%d, cool-bat-decidegc=%d, cold-bat-decidegc=%d, batt-hot-percentage=%d, batt-cold-percentage=%d\n",
			chip->cfg_bat_present_decidegc,
			chip->cfg_hot_bat_decidegc,
			chip->cfg_warm_bat_decidegc,
			chip->cfg_cold_bat_decidegc,
...
static irqreturn_t qpnp_lbc_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	printk("usbin-valid triggered: %d\n", usb_present);
