...
//Start:Reqxxx,liuyang3.wt,20160331,when esd check fail ,not close backlight.		
		if (mfd->panel_info->panel_dead == false){
			mutex_lock(&mfd->bl_lock);
			mdss_fb_set_backlight(mfd, 0);
			mfd->bl_updated = 0;
			...
			pr_info("panel is dead, esd recovery!\n");
			pr_info("brightness level == %d\n", mfd->bl_level);
		}
//End		
	}
