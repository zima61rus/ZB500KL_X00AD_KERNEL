...
//Start:Reqxxx,liuyang3.wt,ADD and MODIFY,20160304,for esd
static void mdss_panel_parse_esd_params(struct device_node *np,
				       struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->status_command_num = (!rc ? tmp : 0);
		pr_info("%s: parse panel status-command-num = %d\n", __func__,ctrl_pdata->status_command_num);
	if (ctrl_pdata->status_command_num > PANEL_STATUS_REG_MAX_NUM){
		pr_err("%s:status_command_num is more than max num %d. \n", __func__, PANEL_STATUS_REG_MAX_NUM);
		return ;
	}
	
	for (i = 0; i< ctrl_pdata->status_command_num; i++){
		sprintf(cmd_key,"qcom,mdss-dsi-panel-status-command%d", i+1);
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->status_cmds[i],
			cmd_key,"qcom,mdss-dsi-panel-status-command-state");
	}

	//mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->status_cmds[1],
	//		"qcom,mdss-dsi-panel-status-command2",
	//			"qcom,mdss-dsi-panel-status-command-state");
	//mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->status_cmds[2],
	//		"qcom,mdss-dsi-panel-status-command3",
	//			"qcom,mdss-dsi-panel-status-command-state");
	
	rc = of_property_read_u32_array(np, "qcom,mdss-dsi-panel-status-value-rlen", ctrl_pdata->status_cmds_rlen, ctrl_pdata->status_command_num);
	if (rc == 0  ){
		pr_info("%s:line %d :parse qcom,mdss-dsi-panel-status-value-rlen:\n", __func__, __LINE__);
		for (i = 0; i < ctrl_pdata->status_command_num; i++ )
			pr_info("ctrl_pdata->status_cmds_rlen[%d] = %d\n", i, ctrl_pdata->status_cmds_rlen[i]);	
	}
	
	rc = of_property_read_u32_array(np, "qcom,mdss-dsi-panel-status-value", &ctrl_pdata->status_value[0][0], ctrl_pdata->status_command_num * PANEL_STATUS_REG_MAX_RET);
	if (rc == 0  ){
		pr_info("%s:line %d :parse qcom,mdss-dsi-panel-status-value:\n", __func__, __LINE__);
		for (i = 0; i < ctrl_pdata->status_command_num ; i++ )
		{
			for (j = 0; j < ctrl_pdata->status_cmds_rlen[i];  j++)
			pr_info("ctrl_pdata->status_value[%d][%d] = 0x%x\n", i, j, ctrl_pdata->status_value[i][j]);	
		}
	}
...
/*
static int mdss_dsi_parse_status_value(struct device_node *np,
		u32 status_value[MDSS_ESD_STATUS_VALUE_LEN], u32 *status_len,
		const char *name)
{
	int num = 0, i;
	int rc;
	struct property *data;
	u32 tmp[MDSS_ESD_STATUS_VALUE_LEN];
	*status_len = 0;
	data = of_find_property(np, name, &num);
	num /= sizeof(u32);
	if (!data || !num || num > MDSS_ESD_STATUS_VALUE_LEN || num % 2) {
...
<3>[  747.216279] ctrl_pdata->status_buf[0].data[0] = 0:

<3>[  747.220780] ctrl_pdata->status_buf[0].data[1] = 0:

<3>[  747.226002] ctrl_pdata->status_buf[1].data[0] = 80:

<3>[  747.230400] ctrl_pdata->status_buf[1].data[1] = 1:

<3>[  747.235396] ctrl_pdata->status_buf[1].data[2] = 82:

<3>[  747.240052] ctrl_pdata->status_buf[1].data[3] = 7:

<3>[  747.245278] ctrl_pdata->status_buf[2].data[0] = 81:

<3>[  747.249672] ctrl_pdata->status_buf[2].data[1] = f3:

<3>[  747.254804] ctrl_pdata->status_buf[2].data[2] = 4:

<3>[  747.259828] ctrl_pdata->status_buf[2].data[3] = 0:
*/
	int i = 0, j =0;
	pr_debug("%s: Read panel status reg:\n",__func__);
	for(i = 0; i < ctrl_pdata->status_command_num; i++){
		for(j = 0; j < ctrl_pdata->status_cmds_rlen[i]; j++){
			if (ctrl_pdata->status_buf[i].data[j] != ctrl_pdata->status_value[i][j]){
				pr_err("ctrl_pdata->status_buf[%d].data[%d] = %x:\n",i , j, ctrl_pdata->status_buf[i].data[j]);
				pr_err("%s: panel status is ERROR!!!\n",__func__);
				return -EINVAL;
			}
		}
	}
	pr_debug("%s: panel status is OK!!!\n",__func__);
	return 1;
}
/*
...
//Esd:Reqxxx,liuyang3.wt,ADD,20160304,for esd
...
//Start:Reqxxx,liuyang3.wt,ADD and Modify,20160304,for esd	
	mdss_panel_parse_esd_params(np,ctrl_pdata);
	...
	if (!rc) {
		if (!strcmp(data, "bta_check")) {
			...
			ctrl_pdata->check_read_status =mdss_dsi_gen_read_status;
		} else if (!strcmp(data, "reg_read_nt35596")) {
			ctrl_pdata->status_mode = ESD_REG_NT35596;
			//ctrl_pdata->status_error_count = 0;
			//ctrl_pdata->status_cmds_rlen = 8;
			//ctrl_pdata->check_read_status = mdss_dsi_nt35596_read_status;
