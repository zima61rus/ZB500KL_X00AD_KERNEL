		...
		struct platform_device *pdev, char *panel_cfg)
{
	...
	char lcm_info[HARDWARE_MAX_ITEM_LONGTH] = "0";//add hardware info for factory mode.
...
//Start:Reqxxx,liuyang3.wt,ADD,20160314,add hardware info for factory mode.
		...
		strcpy(Lcm_name,lcm_info);
	     if (hardwareinfo_set_prop(HARDWARE_LCD, lcm_info) < 0){
			pr_err("[LCD] Failed to hardwareinfo_set_prop\n");
			//return -1;
           }
//End:Reqxxx,liuyang3.wt,ADD,20160314,add hardware info for factory mode.
		return dsi_pan_node;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_err("==>%s: cmd_cfg_cont_splash = %d\n", __func__, cmd_cfg_cont_splash);
