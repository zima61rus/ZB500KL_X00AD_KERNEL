...
#define STATUS_CHECK_INTERVAL_MS  5000  //for debug esd
 ...
 */
 extern bool irq_flag;//add by liuyang.
				...
				unsigned long event, void *data)
{
	...
	if (event == FB_EVENT_BLANK) {
		...
		case FB_BLANK_UNBLANK:
			pr_debug("%s %d:FB_BLANK_UNBLANK\n", __func__, __LINE__);
			pr_debug("mfd->bl_level = %d\n",mfd->bl_level);
			//mdss_fb_set_backlight(mfd, 100);
//Modify by liuyang3,20160712,			
			if (irq_flag) {
				cancel_delayed_work(&pdata->check_status);
			}else{
