...
//Start:Reqxxx,liuyang3.wt,ADD and MODIFY,20160304,for esd
	//int i = 0;
	for (i = 0; i < PANEL_STATUS_REG_MAX_NUM; i++){
		mdss_dsi_buf_alloc(ctrl_dev, &ctrl->status_buf[i], SZ_4K);
	}
//End:Reqxxx,liuyang3.wt,ADD and MODIFY,20160304,for esd	
...
{
//Strart:Reqxxx,liuyang3.wt,Modify,20160304,add for esd.
	int ret = 0;
	int i = 0;
	struct dcs_cmd_req cmdreq;
	for (i = 0; i < ctrl->status_command_num; i++){
		...
		cmdreq.cb = NULL;
		cmdreq.rbuf = ctrl->status_buf[i].data;
		pr_debug("%s:mdss_dsi_cmdlist_put\n ",__func__);
		ret = mdss_dsi_cmdlist_put(ctrl, &cmdreq);
		pr_debug("%s: ret = %d\n",__func__,ret);
	}
	
	return  ret;
//End:Reqxxx,liuyang3.wt,Modify,20160304,add for esd.
}
 ...
 * zero otherwise.
 */
bool irq_flag = false; // add by liuyang3,20160712,
int mdss_dsi_reg_status_check(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (ctrl_pdata->status_cmds[0].link_state == DSI_HS_MODE)
...
irqreturn_t mdss_dsi_isr(int irq, void *ptr)
{
	...
	if (isr & DSI_INTR_ERROR) {
		...
		if (irq_flag){
			disable_irq_nosync(irq); //add for debug.20160511,liuyang
			pr_err("==>no panel, disable DSI_INTR_ERROR irq...\n");
		}
		mdss_dsi_error(ctrl);
	}
