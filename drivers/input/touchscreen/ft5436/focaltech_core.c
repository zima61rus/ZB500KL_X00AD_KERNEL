...
#if CTP_HAVE_KEYS
			if(event->au16_y[i]>=1310 && event->au16_y[i] <= 1390)/*modified by anhengxuan 2016.6.02,center_y(1350)¡À40*/
			{
				key_flag=1;
				if (event->au16_x[i]>=60&&event->au16_x[i] <= 180)/*modified by anhengxuan 2016.6.02,center_key_back(120)¡À60*/
				{
					key_value = KEY_BACK;
					input_report_key(data->input_dev, KEY_BACK, 1);
				}
				else if(event->au16_x[i]>=300 && event->au16_x[i] <= 420)/*modified by anhengxuan 2016.6.02,center_key_home(360)¡À60*/
				{
					key_value = KEY_HOME;
					input_report_key(data->input_dev, KEY_HOME, 1);
				}else if(event->au16_x[i]>=580 && event->au16_x[i] <= 640)/*modified by anhengxuan 2016.6.02,center_key_menu(580)¡À60*/
				{
					key_value = KEY_MENU;
...
//Start:Reqxxx,liuyang3.wt,ADD,20160521,	add hardware info for factory mode.
	fts_read_reg(client, FTS_REG_FW_VER, &version_info);
	sprintf(tp_info,"vendor:Eachopto,ic:FT5436,version:0x%x",version_info);
	err = hardwareinfo_set_prop(HARDWARE_TP,tp_info);
    if (err < 0){
	    pr_err("[TP] Failed to hardwareinfo_set_prop, %d\n",err);
		//return -1;
    }
//End:Reqxxx,liuyang3.wt,ADD,20160521,add hardware info for factory mode.
